...
> 当前为 **0.1.0a2 alpha**。当前开发线不再持有进程、也不再驱动多路复用器，
> 因此早期验收记录里的 tmux/PTY 证据描述的是上一版模型。
> Codex **0.155.1** 通过了已认证的精确恢复冒烟；Claude Code、Pi 与其他原生版本未认证。
> [验收记录](docs/validation/macos-0.1.0a2.md)。此版本尚未发布到 PyPI。

## 从当前源码安装

需要 macOS / Linux 与 **Python 3.11+**，**不需要 tmux**。实际测试过的版本以
兼容记录为准，而非这里的下限。原生 Windows 不支持；可在 Linux/WSL 中尝试。
...
byobu、tmux、screen 还是裸终端都一样：4top 在你正在使用的终端里启动 agent，
自己不创建任何 pane。想让会话活到你合上笔记本之后，就在你已经在用的多路复用器里跑 4top。

`/` 搜索；Ctrl-F 明确触发全文搜索；Esc 清除或取消；空格只读预览；`i` 详情；
`n` 新建；`r` 刷新；`?` 帮助。普通文字匹配支持中文和带引号的词组，多个词按 AND 匹配，
不执行正则或历史内容。全文搜索只读已配置来源，明确报告部分扫描结果。
...
因为 4top 不持有进程，它也不报告存活状态：没有 `LIVE`、`EXIT`、`MISSING`。
一行只表示“可以恢复的会话”，仅此而已；查询失败会明确报错，而不会显示成一台空机器。

两个必须知道的后果：恢复出的 agent 是新进程；同目录多 agent **没有工作树隔离**。
另外 `4top new` 在前台运行 agent——如果当时不在 byobu/tmux 里，它会随终端一起结束。
...
视图是**隔离**的：默认只显示本机，`--host` 是替换而不是把多台机器合并成一张表。
